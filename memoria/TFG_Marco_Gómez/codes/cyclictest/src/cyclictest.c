...
static void rstat_print_stat(struct thread_param *par, int index, int verbose,
                             int quiet);
 ...
 * Return: idle state count on success, negative on error
 */
static int set_deepest_cpu_idle_state(unsigned int cpu,
...
static inline int restore_cpu_idle_disable_state(unsigned int cpu) {
  return -1;
}
static inline void free_cpu_idle_disable_states(void) {}
static inline int set_deepest_cpu_idle_state(unsigned int cpu,
                                             unsigned int state) {
  return -1;
}
...
#else
static int open_msr_file(int cpu) { return -1; }

static int get_smi_counter(int fd, unsigned long *counter) { return 1; }
static int has_smi_counter(void) { return 0; }
#endif
...
static void display_help(int error) {
  ...
  printf(
      ...
      "                           or if CPUSET given, pin threads to that set "
      "of\n"
      "                           processors in round-robin order.\n"
      "                           E.g. -a 2 pins all threads to CPU 2, but\n"
      "                           -a 3-5,0 -t 5 will run the first and fifth "
      "threads\n"
      ...
      "                           Reduce exit from idle latency by limiting "
      "idle state\n"
      "                           up to n on used cpus (-1 disables all idle "
      "states).\n"
      "                           Power management is not suppresed on other "
      "cpus.\n"
      "         --default-system  Don't attempt to tune the system from "
      "cyclictest.\n"
      "                           Power management is not suppressed.\n"
      "                           This might give poorer results, but will "
      "allow you\n"
      "                           to discover if you need to tune the system\n"
      "-d DIST  --distance=DIST   distance of thread intervals in us, "
      "default=500\n"
      "-D       --duration=TIME   specify a length for the test run.\n"
      "                           Append 'm', 'h', or 'd' to specify minutes, "
      "hours or days.\n"
      "-F       --fifo=<path>     create a named pipe at path and write stats "
      "to it\n"
      "-h       --histogram=US    dump a latency histogram to stdout after the "
      "run\n"
      "                           US is the max latency time to be tracked in "
      "microseconds\n"
      "			   This option runs all threads at the same priority.\n"
      "-H       --histofall=US    same as -h except with an additional summary "
      "column\n"
      "	 --histfile=<path> dump the latency histogram to <path> instead of "
      "stdout\n"
      "-i INTV  --interval=INTV   base interval of thread in us default=1000\n"
      "         --json=FILENAME   write final results into FILENAME, JSON "
      "formatted\n"
      ...
      "                           the main thread and not the measurement "
      "threads\n"
      ...
      "	 --policy=NAME     policy of measurement thread, where NAME may be "
      "one\n"
      ...
      "-S       --smp             Standard SMP testing: options -a -t and same "
      "priority\n"
      "                           of all threads\n"
      "	--spike=<trigger>  record all spikes > trigger\n"
      "	--spike-nodes=[num of nodes]\n"
      "			   These are the maximum number of spikes we can "
      "record.\n"
      ...
      "                           without -t default = 1\n"
      "         --tracemark       write a trace mark when -b latency is "
      "exceeded\n"
      ...
      "-x	 --posix_timers    use POSIX timers instead of "
      "clock_nanosleep.\n");
  if (error)
    exit(EXIT_FAILURE);
  exit(EXIT_SUCCESS);
...
static void process_options(int argc, char *argv[], int max_cpus) {
  ...
  for (;;) {
    ...
    int c =
        getopt_long(argc, argv, "a::A::b:c:d:D:F:h:H:i:l:MNo:p:mqrRsSt::uvD:x",
...
static void print_stat(FILE *fp, struct thread_param *par, int index,
  ...
  if (!verbose) {
    ...
    while (stat->cycles != stat->cyclesread) {
      ...
      if (smi)
        diff_smi = stat->smis[stat->cyclesread & par->bufmsk];
...
static int rstat_shm_open(void) {
  ...
  errno = 9;
  fd = shm_open(shm_name, O_RDWR | O_CREAT | O_EXCL,
...
int main(int argc, char **argv) {
  ...
  if (deepest_idle_state >= -1) {
    if (!have_libcpupower_support()) {
      fprintf(stderr, "cyclictest built without libcpupower, "
                      "--deepest-idle-state is not supported\n");
      goto out;
    }
