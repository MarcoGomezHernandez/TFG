...
namespace GeneticPublicConfig
{
    ...
    inline constexpr double V_TH_MARGIN_FACTOR = 0.125;
...
struct GeneticRanges
{
    ...
    {
        constexpr double V_TH_MARGIN_FACTOR = GeneticPublicConfig::V_TH_MARGIN_FACTOR;
        constexpr double EXPECTED_I_MARGIN_FACTOR = GeneticPublicConfig::EXPECTED_I_MARGIN_FACTOR;
        constexpr double G_MIN_FACTOR = GeneticPublicConfig::G_MIN_FACTOR;
        constexpr double K1_MIN = GeneticPublicConfig::K1_MIN;
        constexpr double K1_MAX = GeneticPublicConfig::K1_MAX;
        constexpr double K2_MIN = GeneticPublicConfig::K2_MIN;
        constexpr double K2_MAX = GeneticPublicConfig::K2_MAX;

        const double v_post_range = v_post_max - v_post_min;

        const double e_syn_final_term = GeneticPublicConfig::E_SYN_TERM * v_post_range;
        e_syn = search_phase ? (v_post_max + e_syn_final_term) : (v_post_min - e_syn_final_term);

        const double v_pre_margin = (v_pre_max - v_pre_min) * V_TH_MARGIN_FACTOR;
        const double v_pre_margin_max = v_pre_max + v_pre_margin;
        const double v_pre_margin_min = v_pre_min - v_pre_margin;
        const double v_pre_margin_range = v_pre_margin_max - v_pre_margin_min;

        const double v_post_margin = v_post_range * V_TH_MARGIN_FACTOR;
        const double v_post_margin_max = v_post_max + v_post_margin;
        const double v_post_margin_min = v_post_min - v_post_margin;

        const double expected_i_margin = (expected_i_max - expected_i_min) * EXPECTED_I_MARGIN_FACTOR;
        ...
        if (use_i_fast)
        {
            v_fast = ParamRange(v_pre_margin_min + (v_pre_margin_range * GeneticPublicConfig::V_FAST_MIN_FACTOR),
                                v_pre_margin_max);
            s_fast = ParamRange(GeneticPublicConfig::S_FAST_MIN_FACTOR / v_pre_margin_range,
                                GeneticPublicConfig::S_FAST_MAX_FACTOR / v_pre_margin_range);

            const double sigmoid_fast_max = chemical_sigmoid(s_fast.max, v_fast.min, v_pre_margin_max);
            const double g_fast_max = std::max(std::abs(expected_i_margin_max / ((v_post_margin_max - e_syn) * sigmoid_fast_max)),
                                               std::abs(expected_i_margin_min / ((v_post_margin_min - e_syn) * sigmoid_fast_max)));
        ...
        if (use_i_slow)
        {
            v_slow = ParamRange(v_pre_margin_min,
                                v_pre_margin_max);
            s_slow = ParamRange(GeneticPublicConfig::S_SLOW_MIN_FACTOR / v_pre_margin_range,
                                GeneticPublicConfig::S_SLOW_MAX_FACTOR / v_pre_margin_range);
            ...
            const double m_slow_max_term = K1_MAX * chemical_sigmoid(s_slow.max, v_slow.min, v_pre_margin_max);
            const double m_slow_max = m_slow_max_term / (m_slow_max_term + K2_MIN);

            const double g_slow_max = std::max(std::abs(expected_i_margin_max / ((v_post_margin_max - e_syn) * m_slow_max)),
                                               std::abs(expected_i_margin_min / ((v_post_margin_min - e_syn) * m_slow_max)));
